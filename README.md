...
Fork of `gau` with a few practical fixes and extra features. 🔧

- fixed Wayback issues 🕸️
- loop all Common Crawl indexes 📚
- handle Common Crawl temporary errors more safely ⚠️
- added extra Common Crawl filters with `--ccf`
- added VirusTotal support with `--vt-api-key` or `VT_API_KEY` 🦠
...
getallurls (gau) fetches known URLs from AlienVault's [Open Threat Exchange](https://otx.alienvault.com), the Wayback Machine, Common Crawl, URLScan, and VirusTotal for any given domain. Inspired by Tomnomnom's [waybackurls](https://github.com/tomnomnom/waybackurls).
...
| `--ccf`        | raw Common Crawl filters appended to the query                                              | gau --ccf '=mime:text/html' --ccf '~url:.\*\\.php$' |
...
| `--providers`  | list of providers to use (wayback,commoncrawl,otx,urlscan,virustotal)                       | gau --providers wayback,virustotal                  |
...
| `--vt-api-key` | VirusTotal API key (falls back to `VT_API_KEY`)                                             | gau --vt-api-key $VT_API_KEY example.com            |
...
An example configuration file can be found [here](https://github.com/agmmnn/gau2/blob/main/.gau.toml)
...
```
git clone https://github.com/agmmnn/gau2.git; \
cd gau2/cmd; \
go build; \
...
ohmyzsh's [git plugin](https://github.com/ohmyzsh/ohmyzsh/tree/master/plugins/git) has an alias which maps `gau` to the `git add --update` command. This is problematic, causing a binary conflict between this tool "gau" and the zsh plugin alias "gau" (`git add --update`). There is currently a few workarounds which can be found in this Github [issue](https://github.com/agmmnn/gau2/issues/8).
